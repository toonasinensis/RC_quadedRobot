...
void udp_demo_test(void)
{
	...
	{ 
		...
		err=udp_connect(udppcb,&rmtipaddr,UDP_REMOTE_PORT);//UDP客户端连接到指定IP地址和端口号的服务器
		if(err==ERR_OK)
		{
			err=udp_bind(udppcb,IP_ADDR_ANY,UDP_LOCAL_PORT);//绑定本地IP地址与端口号
			if(err==ERR_OK)	//绑定完成
			{
...
void udp_demo_recv(void *arg,struct udp_pcb *upcb,struct pbuf *p,struct ip_addr *addr,u16_t port)
{
	...
	if (crc32_core((uint8_t *)&udp_receive_data, sizeof(udp_receive_data) / 4 - 1) == udp_receive_data.check_digit)
	{
		udp_send_flag = 1;
...
void udp_demo_senddata(struct udp_pcb *upcb)
{
	...
	if(ptr)
	{
		pbuf_take(ptr,(char*)&udp_send_data,sizeof(udp_send_data)); //将tcp_demo_sendbuf中的数据打包进pbuf结构中
