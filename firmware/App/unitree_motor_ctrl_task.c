...
//	
//   leg[0].thigh_motor.command.Pos = -(stPosWheel1.fpDes - POS_INI_1) * 9.1f / 180.0f * PI + motor1_init_pos;
//   leg[0].knee_motor.command.Pos  = (stPosWheel2.fpDes - POS_INI_2) * 9.1f / 180.0f * PI + motor2_init_pos;
// 	 leg[1].thigh_motor.command.Pos = -(stPosWheel3.fpDes - POS_INI_3) * 9.1f / 180.0f * PI + motor3_init_pos;
//   leg[1].knee_motor.command.Pos  = (stPosWheel4.fpDes - POS_INI_4) * 9.1f / 180.0f * PI + motor4_init_pos;	
//   leg[2].thigh_motor.command.Pos = -(stPosWheel5.fpDes - POS_INI_5) * 9.1f / 180.0f * PI + motor5_init_pos;
//   leg[2].knee_motor.command.Pos  = (stPosWheel6.fpDes - POS_INI_6) * 9.1f / 180.0f * PI + motor6_init_pos;	
//   leg[3].thigh_motor.command.Pos = -(stPosWheel7.fpDes - POS_INI_7) * 9.1f / 180.0f * PI + motor7_init_pos;
//   leg[3].knee_motor.command.Pos  = (stPosWheel8.fpDes - POS_INI_8) * 9.1f / 180.0f * PI + motor8_init_pos;
...
void A1_protect(void)
{
	...
	for(int i = 0;i < 4;i++)
	{
   if(leg[i].knee_motor.feedback.Error != 0||leg[i].thigh_motor.feedback.Error != 0)
	 {
